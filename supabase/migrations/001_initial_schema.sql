...
-- Library policies: public read (active only), admin write
...
-- Employee policies: public read, admin write
...
SELECT
  ...
  'Quezon City Evacuation Center',
  'Team lead for NCR earthquake response'
...
SELECT
  ...
  (SELECT id FROM library_statuses WHERE name = 'Deployed' LIMIT 1),
  'Cebu City Disaster Hub',
  'Medical team stationed at central hub'
...
SELECT
  ...
  'Managing relief goods inventory'
...
SELECT
  ...
  (SELECT id FROM library_statuses WHERE name = 'Deployed' LIMIT 1),
  'Tacloban Response Center',
  'Providing trauma counseling services'
